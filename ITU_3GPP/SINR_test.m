...
%% Computation of P_rx (at the receiver for BD1)
...
%% Computation of Interference at the receiver

bd2_rx_loss = D2D(f,d3);
L_total_interference = UMi_loss + d2d_loss_pingpong + bd2_rx_loss;

P_rx_interference_dbm = P_tx - (L_total_interference + add_loss + slow_fading_umi) + G_t;
P_i = 10.^((P_rx_interference_dbm-30)/10);

%% SINR calculation

SINR_linear = ones(1,length(d3));
for i = 1:length(d3)
%     IN(i) = (P_n + P_i(i));
    SINR_linear(i) = P_rx./P_i(i);
end
